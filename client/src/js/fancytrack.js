...
(function (global, factory) {
  ...
  FancyTrack.prototype = {
      ...
      window.onerror = function (e, url, lineNumber, columnNumber, eObject) {
        ...
        if (window.console && console.log && me.consoleText) {
          if (me.isIE) {
            ...
            console.log.apply(
              window,
              [].concat(me.consoleText).concat(me.consoleArgs),
            );
          }
        }

        let report = {};
        const _report = me.report;
     ...
     */
    ajax(oParams) {
      let sUrl = oParams.url;
      var { method } = oParams;
      const params = oParams.params || {};
      let sData = '';
      const bAsync = true;
      var p;
      const type = oParams.type || 'json';
      const headers = oParams.headers || {};
      ...
      const oBeforeSend = oParams.beforeSend || null;
      const oErrorCallback = oParams.failure || null;
      const oSuccessCallback = oParams.success || null;
      let oXhr = null;
      ...
      const bLocal = window.location.protocol == 'file:';
    ...
    getUserInfo() {
      ...
      me.isIE =
        ...
        /Internet/.test(browser) ||
        /Explorer/.test(browser);
